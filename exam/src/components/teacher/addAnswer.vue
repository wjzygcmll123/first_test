    ...
    <el-table :data="pagination.records" border>
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          <el-button @click="add(scope.row.paperId,scope.row.source,scope.row.examCode)" type="primary" size="small">增加题库</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    add(paperId,source,examCode){ //增加题库
      this.$router.push({path:'/addAnswerChildren',query: {paperId: paperId,subject:source,examCode:examCode}})
    }
  },
};
</script>
